...
df_observed_summaries = df_summaries_obs
df_params_RABC_obs = np.kron(np.ones((3,1)),df_params_RABC_obs)  #triplicate to improve prediction
...
for i in range(len(df_params_RABC_obs)):
    beta = df_params_RABC_obs[i,0]
    gamma = df_params_RABC_obs[i,1]
    initial_status = [N-1,1,0]
    data = SIR_Spreadfunction(beta, gamma, timesteps, risk_matrix, initial_status)
...
currentI_mean = np.mean(currentI_data_sort30, axis=0)

totalI_mean = np.mean(totalI_data_sort30, axis=0)
...
y1 = currentI_mean
y2 = currentI_obs
CI_currentI_mean = 1.96*np.std(currentI_data_sort30, axis=0)/(observedkeep**.5)



plt.fill_between(x, (y1-CI_currentI_mean), (y1+CI_currentI_mean), color='b', alpha=.1)

plt.plot(x, y1, label = "Proposed Predicted Current Infected")
plt.plot(x, y2, '-.',  label = "Observed Current Infected")

plt.xlabel("Time")
plt.ylabel("Current Infected")
plt.legend(loc="lower right")
plt.savefig("Fold2_AverageNetwork_CurrentInfectedFit_realization" + str(realization)+"particles_"+str(para1)+"thre_"+str(para2)+".pdf")
#plt.show()
...
y4 = totalI_obs
CI_totalI_mean = 1.96*np.std(totalI_data_sort30, axis=0)/(observedkeep**.5)
plt.fill_between(x, (y3-CI_totalI_mean), (y3+CI_totalI_mean), color='b', alpha=.1)

plt.plot(x, y3, label = "Proposed Predicted Total Infected")
plt.plot(x, y4, '-.',  label = "Observed Total Infected")

plt.xlabel("Time")
plt.ylabel("Total Infected")
plt.legend(loc="lower right")
plt.savefig("Fold2_AverageNetwork_TotalInfectedFit_realization" + str(realization)+"particles_"+str(para1)+"thre_"+str(para2)+".pdf")
#plt.show()
avenet_runtime = time.time() - start_time
filename = "ABCFold2_SIR_AverageApproximateSummary_realization" + str(realization)+"particles_"+str(para1)+"thre_"+str(para2)+".txt"
